...
- **Images auto-generate for the whole cast** (revised 2026-07-31; they used to require a click per NPC, which meant a cast of fourteen reached play as fourteen name plates). A background pass runs once the pipeline lands and the adventure is `guide_ready`/`active`, walking every NPC that has a description and no images: one generation at a time, through the same chain a player character goes through — `original` → `base` (chroma-key cutout) → `portrait` + `token`. Progress and a Stop button sit above the tabs; the guide stays readable and editable throughout, and a failed image is one missing picture rather than a blocked guide.
