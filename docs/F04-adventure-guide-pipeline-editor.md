...
npcs:        id, adventure_id, name, role ('npc'|'boss'), personality jsonb,
             ...
             faction, voice_id?,
             images jsonb {original,base,portrait,token,tokenBackground}, description text,
...
- All fields autogenerated and editable via the character-creator components in "prefilled" mode.
- **Images are generated on request, never on their own** (revised twice on 2026-07-31: briefly automatic, then reverted on the owner's call — a guide often reuses art it already has, and every image is paid for). Either per NPC on this tab, or in one batch from the guide header, which walks every NPC that has a description and no images: one generation at a time, through the same chain a player character goes through — `original` → `base` (chroma-key cutout) → `portrait` + `token`. Progress and a Stop button sit above the tabs; the guide stays readable and editable throughout, and a failed image is one missing picture rather than a blocked guide.
  - **Crops are framed from the cutout**, not by hand: the transparent base makes the silhouette measurable, so the head is found from it (`features/image/crop-geometry.ts`) and the portrait derives from that frame exactly as the manual tool derives it. The F2 crop tool remains in the panel to re-frame any NPC the measurement gets wrong.
  - Group rows ("the bandits") need no filtering here — stage 6's classifier already deletes them.
  - Image writes deliberately do **not** set `human_edited`: a portrait is not authorship, and marking the cast hand-edited would quietly protect every row from regeneration (§7).
...
- Left list, main overview per location:
  ...
  - **Background image:** generated on request — per location here, or for everything missing at once from the guide header. Both routes go through `generateLocationBackground`, so a hand-triggered plate is framed and worded exactly like a batched one. **Reuse existing** opens the shared library (§5.3.1) instead of paying to draw the place again. Preview with panning simulation; regenerate keeps the last 3.
  ...
  - **Battle map:** drawn FROM the location background — the plate goes to Nano Banana as the only reference and comes back as the same place seen from overhead (`api/location-maps.ts`). Measured 2026-07-31: also handing the model a blank 32×32 lattice pins the ruling to exactly 32 cells but flattens the art (roofs left on, buildings rotated off-axis), while reference-only draws the better map and rules its own grid — so the tile count is read back off the finished image instead (`map-editor/grid-detect.ts`, right on all four test maps). A map needs a background first, and regenerating clears obstacles and spawns, which were painted on different art. Plus a simple editor: place/remove obstacle tiles (blocked movement), spawn markers, and sample tokens from a starter token set in Storage. (Full map authoring tools are out of scope; DMs can upload their own 1024×1024 map image.)

### 5.3.1 Reusable location art (added 2026-07-31)

Every generated background and map is filed in `location_media` with tags from a closed vocabulary
(`features/guide/media-tags.ts`: city, dungeon, forest, tavern, river...). A location can then
**Reuse existing** rather than generate: the picker opens filtered to the tags inferred for that
location and widens on request.

- Tags come free: the model that writes the people-free background brief (§5.3) returns them in the
  same call, validated against the vocabulary. When that call is unavailable, the location's own
  words are matched against the vocabulary instead.
- Reusing shares the storage path rather than copying the object — the storage policy in the
  migration is what lets another adventure read it.
- Maps carry the tile count the detector read, so a reused map arrives with its grid already right.
- Modelled on `battle_maps`, which solved the same problem for the combat lab in July: text[] tags
  with a GIN index, plus public "starter" rows owned by the admin account so a new user opens the
  picker to a stocked shelf.
...
- [ ] NPC images fill in automatically after the pipeline lands, one at a time, with progress and a working Stop; location images still generate only on explicit click. Crops are auto-framed and re-framable through the F2 tool.
