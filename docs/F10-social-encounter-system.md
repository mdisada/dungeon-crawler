...
5. **Broadcast + TTS:** dialogue streams to the text box; TTS fires per completed sentence with the NPC's voice profile, audio chunks stream to clients (F12). Subtitle/portrait active-speaker states follow audio playback.
6. Consistency pass (F7 §6) runs on the dialogue draft before broadcast (fact checks only; tone is free).
