...
import { resolveWarning, resolveWarnings } from '../api/resolve-warning'
...
export function WarningReviewDialog({ open, onOpenChange, queue, onResolved, onEdit }: WarningReviewDialogProps) {
  ...
  const approveAll = async () => {
    ...
    setError(null)
    try {
      await resolveWarnings(queue.map((w) => w.id))
      onOpenChange(false)
      onResolved()
    } catch (err) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogPopup>
        ...
        <DialogFooter>
          {queue.length > 1 && (
            <Button variant="outline" className="mr-auto" disabled={isSaving} onClick={() => void approveAll()}>
