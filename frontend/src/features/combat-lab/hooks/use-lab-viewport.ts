// Copy of features/play/hooks/use-map-viewport.ts (F09 SS9: the Lab must not import from the
// play feature). One addition: `panEnabled` lets paint/measure modes claim the drag gesture.
...
export function useLabViewport(containerRef: React.RefObject<HTMLDivElement | null>, panEnabled: boolean) {
  const [viewport, setViewport] = useState<LabViewport>({ x: 0, y: 0, scale: 0.6 })
  ...
  /** Converts a client-space point into map-pixel coordinates (0..1024). */
