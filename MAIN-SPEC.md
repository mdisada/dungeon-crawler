...
2a. **Code owns identity, sequencing, and liveness; LLMs choose from menus and write prose.** (Story-engine overhaul, 2026-07-23 — see §5.1 and `docs/F08` §12.) Every name the machine keys on — milestone atoms, NPCs, encounter shapes — comes from a registry or a curated menu, never free invention. Where an agent must introduce something new, it *declares* it and code allocates the canonical identity. Story progression, pacing, and referential integrity are deterministic; the model supplies fiction over that skeleton. This principle exists because a cheap default model plus free-text identity produced silently unwinnable adventures (`docs/DECISIONS.md`, 2026-07-23).
...
### 1.1a Where this sits on the railroad ↔ sandbox axis

Assessed 2026-07-23, after the story-engine overhaul: **~4 on a 1–10 scale** (1 = railroad,
10 = sandbox). Deliberate, and worth stating so it is chosen rather than drifted into.

| Axis | Position | Mechanism |
| --- | --- | --- |
| *What* you must accomplish, and in what order | Railroad (2) | Objectives are a strictly sequential ladder — one `active` at a time, revealed in authored order |
| *Whether* you engage at all | Railroad (2) | One `is_entry` contract opens the adventure; an unanswered offer is eventually forced (§F08 §12.5) |
| *How* you accomplish it | Sandbox (7) | `any`-combinator predicates honour multiple resolutions; off-script actions become real ad-hoc encounters; ingredients are toys, not gates |
| *What the world becomes* | Sandbox (7–8) | Player theories can be canonized; dispositions shift; the suspicion tally can commit the BBEG; the antagonist advances off-screen |
| *How it ends* | Sandbox (6) | 3–5 candidate endings scored from objective outcomes (including failures), NPC states, and dials. The final objective is a designated **climax** framed as the peak whatever its form; the winning ending publishes its prose to the player once (F08 §12.10) |
| *Story type* | Sandbox (6) | The Loop Classifier can swap the active core loop mid-play |

In tabletop terms: an **Adventure Path** — authored spine, free play within scenes, multiple
endings. The overhaul moved us *down* this scale (the Progress Director, forced offers, and
guaranteed routes all exist to ensure the spine gets traversed); the trade was accepted because
what preceded it was not freedom but fragility — stories that silently stalled.

**Backlog — raising the ceiling** (not scheduled; recorded so the constraint is visible):

1. **Parallel active objectives.** The single biggest factor. The ladder is sequential by
   construction (`reveal_state`, one `objectives.currentId`). Allowing 2–3 live objectives in
   player-chosen order moves this to ~6 alone. The core-loop stack already models
   suspend/resume, so half the machinery exists. Note this changes what "reachable" means for
   the F08 §12.7 reachability lint — decide before that lands.
2. **Optional / side objectives** off the spine (today every objective is load-bearing).
3. **A sandbox preset** for the Progress Director — thresholds are already DM-tunable
   (`dm.settings.directorThresholds`); a high-threshold preset dials intervention down with no
   code change.
4. **Declining branches** rather than forces — a refused entry contract re-weaves into a
   *different* contract instead of being overridden.
...
| **Beat Planner** | Generates the *next beat only* of the active loop: goals, exit conditions, ingredient requests. **Two calls** since 2026-07-23: (1) plan the beat and *declare* its new local atoms + any cast it needs created; (2) map outcome tiers onto milestones from a schema-enum'd menu of registered atoms. Splitting removed the circularity that forced free-text milestone invention | Loop pivot accepted; beat completed; Progress Director re-plan |
...
| **Objective Judge** | On beat exit/spent with an active objective uncredited, decides whether the *fiction* already completed it; must cite a verbatim evidence quote and pick an atom from an enum | Beat exit or spent, deterministic path credited nothing |
| **Archivist (Scene Ledger)** | Closes a phase: milestones observed, NPC states, dial nudges, contributions, digest | Encounter/objective/conversation phase close |
| **Encounter Designer** | Also picks the encounter's **template + twist axis** from a curated menu (§5.1) so shapes vary and rescues never read generic | Beat open; guaranteed-route skinning |
...
### 5.1 Story-spine engines (added 2026-07-23)

The deterministic half of principle §1.1(2a). All pure, unit-tested, in `packages/rules/src/`
and synced to `supabase/functions/_shared/` by `scripts/sync-guide-shared.mjs`. Full behaviour
in `docs/F08` §12.

| Engine | Responsibility |
| --- | --- |
| **Atom Registry** (`story/atoms.ts`) | The single naming authority for milestone atoms. Canonicalizes (case/punctuation/apostrophes), resolves a proposal to an authored atom, allocates identities for atoms an agent *declares*. **Auto-repair is canonical equality only** — fuzzy matching is suggestion material, because antonyms sit 1–2 edits apart (`guards_averted`/`guards_alerted`) and token reorder flips subject and object |
| **Progress Director** (`story/director.ts`) | The pacing spine. Per-turn counters → at most one action from a single monotonic ladder: `nudge` → `reveal` → `replan_beat` → `guaranteed_route` → `fail_forward`, plus an orthogonal offer-pressure track with backoff and a terminal forced start. Seeded jitter (telegraph, don't schedule). Thresholds DM-tunable |
| **Route Health** (`session/route-health.ts`) | Beat liveness — `healthy`, `stillborn`, `spent`, `missing`. Replaced a detector that could only see beats whose encounter had *opened*, and was therefore blind to encounters that can never open |
| **Guaranteed Route** (`guide/guaranteed-route.ts`) | Per objective, the minimal atom set that provably satisfies its predicate (cheapest `any` branch, full `all` union), wrapped as a playable rescue encounter. Property-tested against `evaluatePredicate` |
| **Encounter Templates** (`story/templates-encounter.ts`) | 14 curated mechanical shapes across skill-challenge / social / puzzle / combat, each requiring a **twist axis** (timer, terrain, moral choice, secondary objective). Menu-choice with anti-repeat; the random-table answer to generic gameplay |
| **Staging** (`story/staging.ts`) | Who can take a stage right now (live NPC state over authored `initial_state`), and name→id resolution restricted to the living |
| **Climax + combat floor** (`session/beats.ts`, added 2026-07-24) | The final objective (once an arc exists behind it) is a designated climax beat, framed as the peak whatever its form — a boss fight, an escape, a reckoning — never forced to combat. A boss combat opens itself; a social/skill finale is played. Every adventure gets **1–3 fights, at least one** (`COMBAT_FLOOR`/`COMBAT_BUDGET`): guidance persuades, code forces only at the climax |
| **Ending presentation** (`session/progress.ts`, 2026-07-24) | The committed ending publishes its climax prose to the player **once** — directly (not via a fragile second narrator pass that died on worker limits), consistency-checked structurally, atomically claimed so overlapping passes can't triple-publish. `state:alive` excludes `absent`, so a departed boss no longer flips a won story onto a defeat ending |
...
| **Progress Director** | *The pacing authority (2026-07-23).* Runs on **every** player turn — the one hook every turn passes — maintains story-health invariants and escalates a single ladder when the spine stops moving. Replaced three uncoordinated ladders (stuck hints, dead-table promotion, idle nudge), none of which could advance the story. Cheap by construction: pure reads plus a counter commit; an LLM call only when a rung fires |
...
| **Adventure Guide content** | Chapters, scenes (hidden scaffolding), **objectives** (short player-facing text, hidden description, structured completion predicates, reveal order, **terminal `outcome` (completed/failed) and code-authored `guaranteed_route`** — 2026-07-23), NPCs (personality, stat block ref, voice ref, faction, `initial_state` alive/dead/absent, images), locations (description, background image, grid map + tokens), boss phase definitions |
| **Story atoms** | The canonical milestone-atom registry (`story_atoms`): unique `slug` per adventure, `kind` (flag/event/fact), `scope` (spine = from an objective predicate, local = declared by a beat), `label` (the exact text predicates match on), source row. Every progression name resolves here (§1.1(2a)) |
...
**Story-spine overhaul (2026-07-23, `docs/F08` §12):** atom registry, Progress Director + one escalation ladder, route health, guaranteed routes, fail-forward outcomes, encounter templates. **Climax + ending pass (2026-07-24, §12.10):** the final objective is a designated climax framed as the peak (any form, combat never forced); every adventure gets 1–3 fights with at least one; the committed ending publishes to the player once. First natural (un-driven) one-shot completion on record. Invariants listed in F08 §12.8.
...
| **Unwinnable stories** (realized 2026-07-22) | A beat can reference an unstageable NPC, a planner can invent an atom nothing awards, and an objective can have no live route — each silently freezes the spine forever. Mitigated by the F08 §12 spine: registry-gated atoms, plan-time stageability, route-health detection, guaranteed routes, and a bounded ladder that always terminates an objective |
| **Agency loss from that mitigation** | The Director forces offers and can fail objectives forward. Bounded by: high thresholds, full-AI only for the terminal rungs (assist gets DM proposals), kill-switch flags, and rescue-rung firings tracked as *lab anomalies* — a healthy run should never reach them. See §1.1a for the axis position and the backlog to raise it |
